...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  else if(strcmp(msg, "pillarsSolved") == 0) {
    onPillarsSolved();
  }
   else if(strcmp(msg, "ankhSolved") == 0) {
    onAnkhSolved();
  }
   else if(strcmp(msg, "pillarsReset") == 0) {
    onPillarsReset();
  }
  else if(strcmp(msg, "ankhReset") == 0) {
    onAnkhReset();
  }
}
...
void mqttLoop() {
  ...
  while (!MQTTclient.connected()) {
    ...
    if (MQTTclient.connect(deviceID)) {
      ...
      snprintf(topic, 32, "ToHost/Statue", deviceID);
      snprintf(msg, 64, "CONNECTED", deviceID);
      MQTTclient.publish(topic, msg);
      
      // Subscribe to topics meant for this device
      snprintf(topic, 32, "ToDevice/Statue", deviceID);
